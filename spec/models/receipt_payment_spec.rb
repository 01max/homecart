...
RSpec.describe ReceiptPayment do
  ...
  it "declares payment categories" do
    payment = create(:receipt_payment)
  ...
  it "requires unique positions within a receipt" do
    payment = create(:receipt_payment)
    duplicate = create(:receipt_payment, receipt: payment.receipt, position: 2)
  ...
  it "requires a positive integer amount" do
    payment = create(:receipt_payment).tap { |record| record.amount_cents = 0 }
