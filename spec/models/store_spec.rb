...
RSpec.describe Store do
  ...
  it "owns source documents and receipts" do
    ...
    receipt = create_receipt(store: store, source_document: source_document)
  ...
  it "requires identifiers to be an object" do
    store = described_class.new(retail_brand: create_retail_brand, location_name: "Location", channel: "physical", identifiers: [])

    expect(store).not_to be_valid
  end

  it "requires location names to be unique per brand and channel" do
    store = create_store(location_name: "Location", channel: "physical")
