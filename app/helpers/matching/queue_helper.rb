module Matching
  module QueueHelper
    def matching_queue_date_range(group)
      return t("matching.queue.index.empty_value") if group.first_purchased_at.blank? && group.last_purchased_at.blank?
      ...
      t("matching.queue.index.date_range", first: first_date, last: last_date)
    end
    ...
    def matching_queue_price_context(price_context)
      [
        matching_queue_money_values("matching.queue.index.price_context.total", price_context.total_cents),
        matching_queue_money_values("matching.queue.index.price_context.unit", price_context.unit_price_cents)
    ...
    def matching_queue_quantity_context(price_context)
      ...
      [
        matching_queue_values("matching.queue.index.quantity_context.quantities", quantities),
        matching_queue_values("matching.queue.index.quantity_context.units", units)
      ].compact.to_sentence.presence || t("matching.queue.index.empty_value")
    end
